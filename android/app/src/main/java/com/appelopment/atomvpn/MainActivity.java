package com.appelopment.atomvpn;
...
public class MainActivity extends FlutterActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // Check VPN status when the app is restarted
    ...
    private void checkVPNStatusOnLaunch() {
        String vpnStatus = OpenVPNService.getStatus();
        if (vpnStatus != null && !vpnStatus.isEmpty()) {
            Log.d(TAG, "VPN status on app launch: " + vpnStatus);
            sendVPNStatusToFlutter(vpnStatus);
        }
    }

    private void sendVPNStatusToFlutter(String vpnStatus) {
        if (vpnControlMethod != null) {
            vpnControlMethod.invokeMethod("vpnStatusOnLaunch", vpnStatus);
        }
    }
        ...
        vpnControlMethod.setMethodCallHandler((call, result) -> {
            switch (call.method) {
                ...
                case "stage":
                    ...
                    break;
                case "check_vpn_status_on_launch":
                    boolean isVpnActive = isVPNConnected();
                    if (isVpnActive) {
    ...
    // Method to check if VPN is still active
